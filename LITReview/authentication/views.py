        ...
        if "add_followed" in request.POST:
            ...
            if add_followed_form.is_valid():
                ...
                try:
                    ...
                    # Vérification user ne s'abonne pas à lui-même
                    if followed_user == user:
                        message = "Vous ne pouvez pas vous suivre vous même"
                    # Si l'utilisateur ajoute quelqu'un déjà présent dans sa liste d'abonnement
                    elif aModels.UserFollows.objects.filter(user=user, followed_user=followed_user).exists():
                        message = f"{followed_user.username} est déjà présent dans votre liste d'abonnement"
                    else:
